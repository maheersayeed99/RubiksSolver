...
class Cube {
  ...
  O,O,O,G,G,G,R,R,R,B,B,B,
    ...
    generateGraph(actions) {
        ...
        //3d -------------------------------------------------
        //2d ----------------------------------------------         SUPER IMPORTANT takes a list of possible moves
        for (var face of tempcube.cubeletFaces) {                   
            var pos2d = face.getCurrentPosition2D(this);            // get coordinate of face
            this.graph2d.push(new Knot({                            // add initial nodes to graph
                pos: pos2d,
            }));
        }
    ...
    pathfind2d(dest) {
        var misplacedface = this.cubeletFaces.find(f => f.getStartPosition2D(this).equals(dest));   // find cubelet that goes in that spot
        var start = this.graph2d.find(k => k.pos.equals(misplacedface.getCurrentPosition2D(this))); // location of said cubelet
        var goal = this.graph2d.find(k => k.pos.equals(misplacedface.getStartPosition2D(this)));    // intended loaction of said cubelet
...
function pathfind(start, dest, graph) {                                     // THIS IS DJIKSTRA
    for (var knot of graph) {
        knot.cost = Number.MAX_VALUE;   // set all costs to infinity
        knot.predecessor = null;        // set all predecessors to null
    }
    start.cost = 0;                     // make first node 0
    var explored = [];                  // visited is empty
    var frontier = [start];             // bfs queue?
    while (frontier.length > 0) {
        ...
        var current = frontier.splice(smallest, 1)[0];          // WHAT IS SPLICE??
                                                                // current is a node
        if (current == dest) {
            break;
        }
        for (var edge of current.edges) {                               // edge is not a node, edge.target is
    ...
    return traceback(start, dest);                          // now go backtrack with the parent tree
}
function traceback(start, destination) {                    // pretty self explanatory
    var res = [];
